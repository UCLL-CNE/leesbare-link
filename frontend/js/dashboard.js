const deleteLink = async (link) => {
    const response = await fetch(`${host}/${link.mapping}`, {
...
const loadLinks = async () => {
    ...
    if (response.status !== 200) {
        ...
        links.forEach((link) => {
            ...
            mappingCell.innerHTML = `<a target="_blank" href="${host}/${link.mapping}">${host}/${link.mapping}</a>`;
...
const createLink = async (link, mapping) => {
    const response = await fetch(`${host}/${mapping}`, {
